...
namespace Sirenix.OdinInspector.Editor
{
	...
	public class MemberDefinedAttributeProcessor : OdinAttributeProcessor
	{
		...
		{
			if (!MemberAttributesCache.TryGetValue(member, out var value))
			{
				value = member.GetAttributes<Attribute>(inherit: true).ToList();
				MemberAttributesCache.Add(member, value);
			}
