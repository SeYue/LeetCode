...
namespace Sirenix.OdinInspector.Editor
{
	...
	public sealed class PropertyContextContainer
	{
		...
		private bool TryGetGlobalConfig<T>(string key, out PropertyContext<T> context, out bool isNew)
		{
			...
			Dictionary<string, object> dictionary = globalContexts;
		...
		{
			if (TryGetGlobalConfig(key, out PropertyContext<T> context, out bool isNew) && isNew)
			{
		...
		/// </summary>
		[Obsolete("Use local fields in the drawer instead.", true)]
		public bool Get<TValue>(OdinDrawer drawerInstance, string key, out TValue context) where TValue : class, new()
		{
			throw new NotSupportedException();
		}
		...
		{
			GlobalPersistentContext<TValue> context2;
			bool result = PersistentContext.Get(TwoWaySerializationBinder.Default.BindToName(drawer.GetType()).GetHashCode(), TwoWaySerializationBinder.Default.BindToName(property.Tree.TargetType).GetHashCode(), property.Path.GetHashCode(), new DrawerStateSignature(property.RecursiveDrawDepth, InlineEditorAttributeDrawer.CurrentInlineEditorDrawDepth, property.DrawerChainIndex).GetHashCode(), key, out context2);
			context = LocalPersistentContext<TValue>.Create(context2);
			return result;
		}
		...
		{
			Dictionary<string, object> dictionary = otherContext.globalContexts;
			otherContext.globalContexts = globalContexts;
			globalContexts = dictionary;
		}
	}
}
